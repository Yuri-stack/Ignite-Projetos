...
export function FormAddImage({ closeModal }: FormAddImageProps): JSX.Element {
  ...
  const mutation = useMutation<void, unknown, FormValues, unknown>(async formData => {
    await api.post('api/images', formData) // TODO MUTATION API POST REQUEST,
  },
    {
      onSuccess: () => {
        queryClient.invalidateQueries('images') // TODO ONSUCCESS MUTATION
      }
    }
  );
  ...
  const onSubmit = async (data: Record<string, string>): Promise<void> => {
    try {
      if (!imageUrl) {        // TODO SHOW ERROR TOAST IF IMAGE URL DOES NOT EXISTS
        toast({
          title:
